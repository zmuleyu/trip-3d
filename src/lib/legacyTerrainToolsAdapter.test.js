...
function setup({ route = { name: '测试线路', waypoints: [{ id: 'a' }, { id: 'b' }] }, supported = true, deferTerrain = false } = {}) {
  ...
  const ports = {
    poster: { unavailable: vi.fn(), pending: vi.fn(), captureImage: vi.fn(async () => 'image'), render: vi.fn(() => 'canvas'), download: vi.fn() },
    flyover: {
      routeInsufficient: vi.fn(), isSupported: () => supported, unsupported: vi.fn(), durationFor: () => 12,
    ...
    terrain: {
      ...
      refreshStaticShadow: vi.fn(), hideLoading: vi.fn(), reportReady: vi.fn(), resolveWaiters: vi.fn(),
    },
    hud: { regenerate: vi.fn(), setVisible: vi.fn(), triggerScan: vi.fn() },
    camera: { cancelMotion: vi.fn(), startTour: vi.fn(() => true), stopTour: vi.fn() },
    advanced: { sync: vi.fn() },
  }
...
describe('LegacyTerrainToolsAdapter', () => {
  ...
  it('keeps procedural terrain, HUD, advanced settings, camera and scan behind ports without idle RAF work', () => {
    ...
    expect(ports.hud.regenerate).toHaveBeenCalledOnce()
    expect(requestLegacyFrames).not.toHaveBeenCalled()

    adapter.setHudVisible(false)
    adapter.regenerateHud()
    adapter.syncAdvancedSettings()
    adapter.triggerScan()
    expect(adapter.startTour()).toBe(true)
    adapter.stopTour()

    expect(ports.hud.setVisible).toHaveBeenCalledWith(false)
    expect(ports.hud.triggerScan).toHaveBeenCalledOnce()
    expect(ports.advanced.sync).toHaveBeenCalledOnce()
    expect(ports.camera.stopTour).toHaveBeenCalledOnce()
    expect(requestLegacyFrames).toHaveBeenCalledTimes(2)
  })
